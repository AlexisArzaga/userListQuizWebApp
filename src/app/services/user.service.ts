...
export class UserService {
  ...
  public login(email:string , password:string)
  {
   ...
   return this.httpClient.post(Constants.BASE_URL + "user/Login", body);
  }
  ...
  public register(fullname:string,email:string , password:string, roles:string[])
  {
   ...
   return this.httpClient.post(Constants.BASE_URL + "user/RegisterUser", body);
  }
   ...
   return this.httpClient.get<ResponseModel>(Constants.BASE_URL+"user/GetAllUser",{headers:headers}).pipe(map(res=>{
     ...
     if(res.responseCode==ResponseCode.OK)
     {
          ...
          res.dateSet.map((x:User)=>{
              userList.push(new User(x.userId,x.fullName,x.email,x.userName,x.roles, x.dateCreated));
          })
          }
         }
         return userList;
